...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Local dev convenience: proxy API calls to the FastAPI backend
      // (run separately via `uvicorn app.main:app --port 8000` in
      // backend/) so the dashboard can just fetch('/api/...') with no
      // CORS setup needed. Production builds instead use
      // VITE_API_BASE_URL (see src/lib/api.ts) or nginx's /api proxy
      // (see nginx.conf) — this block only affects `pnpm run dev`.
      proxy: {
        '/api': { target: process.env.VITE_BACKEND_PROXY_TARGET || 'http://localhost:8000', changeOrigin: true },
        '/healthz': { target: process.env.VITE_BACKEND_PROXY_TARGET || 'http://localhost:8000', changeOrigin: true },
      },
    },
    preview: {
      host: process.env.FIGMA_DEV_SERVER_HOST || '0.0.0.0',
